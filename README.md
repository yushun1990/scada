...
五张状态图的原始画布、透明边界和水泵位置可以略有不同。图片首次加载后会自动执行统一处理：

1. 扫描透明通道并定位实际可见的水泵边界；
2. 去除原图多余透明边距；
3. 使用单一缩放因子将水泵内容统一为 660px 高；
4. 水平居中放入固定的 `512 × 720` 透明设计画布；
5. 将处理结果缓存在浏览器中，后续状态切换直接复用。

归一化过程只进行等比例缩放和平移，不分别拉伸宽度与高度，因此不会改变原图片形状。若 PNG 加载失败，当前实现会自动回退到同源 SVG 生成的状态图。
