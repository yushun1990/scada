...
try {
  ...
  assert.equal(await page.getByLabel('内部矩形 图元类型', { exact: true }).isDisabled(), true)
  const opacity = page.locator('.component-layer-inspector .property-field').filter({ hasText: '透明度' }).locator('input')
  assert.equal(await opacity.isEnabled(), true, 'non-geometric properties remain editable')
  ...
  assert.equal(await page.getByLabel('内部矩形 图元类型', { exact: true }).isEnabled(), true)
  assert.equal(await opacity.inputValue(), '0.6', 'ungroup retains configured child properties')
