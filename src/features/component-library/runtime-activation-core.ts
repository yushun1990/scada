...
type UserComponentActivationControllerOptions = {
...
/**
 * Own the complete user-registration slice of one live registry.
 *
 * The controller validates the complete next registry before mutating the live
 * registry, then replaces only the user types it previously installed.
 */
export function createUserComponentActivationController({
  registry,
  builtInRegistrations,
  createRegistration,
}: UserComponentActivationControllerOptions) {
  const builtInTypes = new Set(
    builtInRegistrations.map((registration) => registration.definition.type),
  )
  ...
  function replace(
    ...
    for (const entry of readyEntries) {
      ...
      try {
        registrations.push(createRegistration(entry))
      } catch (error) {
        diagnostics.push({
          packageId: entry.id,
          componentType,
          ...
          message: error instanceof Error
            ? error.message
            : 'Invalid user component registration',
        })
      }
    }
    ...
    for (const componentType of activeUserTypes) {
      registry.unregister(componentType)
    }
  ...
  return {
    replace,
    getActiveTypes: () =>
      [...activeUserTypes].sort((left, right) => left.localeCompare(right)),
  }
}
